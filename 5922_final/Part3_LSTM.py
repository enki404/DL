...
df = pd.read_csv(str(path+"\Labeled_News_Data_from_API_all.csv"))
...
NumCols=df.shape[1]
# This is vocab
ColumnNames=list(df.columns[1:NumCols])
...
# normalize index number data between 0 and 1
#X = preprocessing.normalize(X)
y = np.array([df.iloc[:,0]]).T

x_train,x_test,y_train,y_test = train_test_split(X,y, test_size = 0.2, shuffle= True, random_state=123)
#x_train,x_val,y_train,y_val = train_test_split(x_train1,y_train1, test_size = 0.1, shuffle= True, random_state=66)
...
model = tf.keras.models.Sequential([
 ...
 #   tf.keras.layers.Dropout(0.5),
  #  tf.keras.layers.LSTM(4),
    ...
    tf.keras.layers.BatchNormalization(),
   # tf.keras.layers.Reshape((-1,4,4), input_shape=(64,)),
   # tf.keras.layers.Flatten(),
    tf.keras.layers.Dropout(0.5),
   # tf.keras.layers.Reshape((4,4)),
   # tf.keras.layers.LSTM(8,kernel_regularizer=tf.keras.regularizers.L1L2(l1=1e-3, l2=1e-3)),
...
Hist=model.fit(x_train, y_train, batch_size= 32, epochs=2500,validation_data=(x_test,y_test), shuffle=True)
...
plt.title('LSTM Accuracy Graph',fontsize=15)
#plt.ylim([0.5,1])
...
plt.title('LSTM Loss Graph',fontsize=15)
#plt.ylim([0.5,1])
...
ax.set_title('Confusion Matrix: Part3_LSTM',fontsize = 36) 
...
cm1_disp.plot()
cm1_disp.ax_.set_title("Part3 LSTM Confusion Matrix",fontsize=35)
